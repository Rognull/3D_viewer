...
void Model::RotationX(double angle) {
  for (unsigned i = 0; i < vertex_vector_.size(); i += 3) {
    double temp_y = vertex_vector_[i + 1];
    double temp_z = vertex_vector_[i + 2];
    vertex_vector_[i + 1] = cos(angle) * temp_y + sin(angle) * temp_z;
    vertex_vector_[i + 2] = -sin(angle) * temp_y + cos(angle) * temp_z;
  }
}

void Model::RotationY(double angle) {
  for (unsigned i = 0; i < vertex_vector_.size(); i += 3) {
    double temp_x = vertex_vector_[i];
    double temp_z = vertex_vector_[i + 2];
    vertex_vector_[i] = cos(angle) * temp_x + sin(angle) * temp_z;
    vertex_vector_[i + 2] = -sin(angle) * temp_x + cos(angle) * temp_z;
  }
}
...
void Model::RotationZ(double angle) {
  for (unsigned i = 0; i < vertex_vector_.size(); i += 3) {
    double temp_x = vertex_vector_[i];
    double temp_y = vertex_vector_[i + 1];
    vertex_vector_[i] = cos(angle) * temp_x - sin(angle) * temp_y;
    vertex_vector_[i + 1] = sin(angle) * temp_x + cos(angle) * temp_y;
  }
}
...
void Model::SettingToCenter() {
  ...
  for (unsigned i = 0; i < vertex_vector_.size(); i += 3) {
    vertex_vector_[i] -= centX;
    vertex_vector_[i + 1] -= centY;
    vertex_vector_[i + 2] -= centZ;
  }
}
void Model::SetScale(double scale) {
  for (unsigned i = 0; i < vertex_vector_.size(); i++) {
    vertex_vector_[i] *= scale;
  }
}
...
std::pair<double *, unsigned *> Model::GetArr() {
  ...
  SetScale(scale);
  return std::pair<double *, unsigned *>(vertex_vector_.data(),
                                         polygons_vector_.data());
}
